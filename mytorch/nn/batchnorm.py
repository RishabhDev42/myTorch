...
class BatchNorm1d:
    ...
    def forward(self, Z, eval=False):
        ...
        self.M = (1/self.N)*np.sum(self.Z, axis=0)  # TODO: Calculate mini-batch mean
        self.V = (1/self.N)*np.sum(np.square(self.Z-self.M), axis=0)  # TODO: Calculate mini-batch variance
    ...
    def backward(self, dLdBZ):
        ...
        dNZdM = -(np.power(self.V+self.eps,-0.5))-0.5*(self.Z-self.M)*np.power(self.V+self.eps,-1.5)*((-2/self.N)*np.sum(self.Z-self.M,axis=0))  # TODO: Compute derivative of normalized input with respect to mean.
        dLdM = np.sum(dLdNZ,axis=0)*dNZdM  # TODO: Compute gradient of loss with respect to mean.

        dLdZ = dLdNZ*(np.power(self.V+self.eps,-0.5))+dLdV*((2/self.N)*(self.Z-self.M))+(1/self.N)*dLdM  # TODO: Compute gradient of loss with respect to the input.
        return dLdZ
